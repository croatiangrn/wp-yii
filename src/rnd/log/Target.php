...
abstract class Target extends Component
{
	 ...
	 *
	 * @see \rnd\helpers\ArrayHelper::filter()
	 */
	...
	public function collect($messages, $final)
	{
		...
		if ($count > 0 && ($final || $this->exportInterval > 0 && $count >= $this->exportInterval)) {
			if (($context = $this->getContextMessage()) !== '') {
				$this->messages[] = [$context, Logger::LEVEL_INFO, 'application', RND_BEGIN_TIME];
			}
	...
	public function getMessagePrefix($message)
	{
		...
		$request = Rnd::$app->getRequest();
		$ip = $request instanceof Request ? $request->getUserIP() : '-';

		/* @var $user \rnd\web\User */
		$user = Rnd::$app->has('user', true) ? Rnd::$app->get('user') : null;
		...
		/* @var $session \rnd\web\Session */
		$session = Rnd::$app->has('session', true) ? Rnd::$app->get('session') : null;
