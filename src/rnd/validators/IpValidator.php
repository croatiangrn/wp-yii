...
class IpValidator extends Validator
{
	...
	public function init()
	{
		...
		if ($this->message === null) {
			$this->message = Rnd::t( '{attribute} must be a valid IP address.', 'rnd');
		}
		if ($this->ipv6NotAllowed === null) {
			$this->ipv6NotAllowed = Rnd::t( '{attribute} must not be an IPv6 address.', 'rnd');
		}
		if ($this->ipv4NotAllowed === null) {
			$this->ipv4NotAllowed = Rnd::t( '{attribute} must not be an IPv4 address.', 'rnd');
		}
		if ($this->wrongCidr === null) {
			$this->wrongCidr = Rnd::t( '{attribute} contains wrong subnet mask.', 'rnd');
		}
		if ($this->noSubnet === null) {
			$this->noSubnet = Rnd::t( '{attribute} must be an IP address with specified subnet.', 'rnd');
		}
		if ($this->hasSubnet === null) {
			$this->hasSubnet = Rnd::t( '{attribute} must not be a subnet.', 'rnd');
		}
		if ($this->notInRange === null) {
			$this->notInRange = Rnd::t( '{attribute} is not in the allowed range.', 'rnd');
		}
	}
