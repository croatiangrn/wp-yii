...
class NumberValidator extends Validator
{
	...
	public function init()
	{
		...
		if ($this->message === null) {
			$this->message = $this->integerOnly ? Rnd::t( '{attribute} must be an integer.', 'rnd')
				: Rnd::t('{attribute} must be a number.', 'rnd' );
		}
		if ($this->min !== null && $this->tooSmall === null) {
			$this->tooSmall = Rnd::t('{attribute} must be no less than {min}.', 'rnd');
		}
		if ($this->max !== null && $this->tooBig === null) {
			$this->tooBig = Rnd::t('{attribute} must be no greater than {max}.', 'rnd');
		}
	}
	...
	protected function validateValue($value)
	{
		if (is_array($value) || is_object($value)) {
			return [Rnd::t('{attribute} is invalid.', 'rnd'), []];
		}
		$pattern = $this->integerOnly ? $this->integerPattern : $this->numberPattern;
		if (!preg_match($pattern, StringHelper::normalizeNumber($value))) {
			return [$this->message, []];
