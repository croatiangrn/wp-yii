...
class DateValidator extends Validator
{
	 ...
	 *
	 * If this property is not set, the default value will be obtained from `Rnd::$app->formatter->dateFormat`, see [[\yii\i18n\Formatter::dateFormat]] for details.
	...
	public function init()
	{
		...
		if ($this->message === null) {
			$this->message = Rnd::t('The format of {attribute} is invalid.', 'app' );
		}
		if ($this->format === null) {
			if ($this->type === self::TYPE_DATE) {
				$this->format = Rnd::$app->formatter->dateFormat;
			} elseif ($this->type === self::TYPE_DATETIME) {
				$this->format = Rnd::$app->formatter->datetimeFormat;
			} elseif ($this->type === self::TYPE_TIME) {
				$this->format = Rnd::$app->formatter->timeFormat;
			} else {
		...
		if ($this->timeZone === null) {
			$this->timeZone = Rnd::$app->timeZone;
		}
		if ($this->min !== null && $this->tooSmall === null) {
			$this->tooSmall = Rnd::t('yii', '{attribute} must be no less than {min}.');
		}
		if ($this->max !== null && $this->tooBig === null) {
			$this->tooBig = Rnd::t('yii', '{attribute} must be no greater than {max}.');
		}
	...
	private function parseDateValueIntl($value, $format)
	{
		...
		if ($parsedDate === false || $parsePos !== mb_strlen($value, Rnd::$app ? Rnd::$app->charset : 'UTF-8')) {
			return false;
		}
