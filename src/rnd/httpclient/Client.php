...
class Client extends Component
{
     ...
     */
    private $_transport = 'rnd\httpclient\StreamTransport';
    ...
    public function getTransport()
    {
        if (!is_object($this->_transport)) {
            $this->_transport = Rnd::createObject($this->_transport);
        }
    ...
    public function getFormatter($format)
    {
        static $defaultFormatters = [
            self::FORMAT_JSON => 'rnd\httpclient\JsonFormatter',
            self::FORMAT_URLENCODED => [
                'class' => 'rnd\httpclient\UrlEncodedFormatter',
            ...
            self::FORMAT_XML => 'rnd\httpclient\XmlFormatter',
        ];
        ...
        if (!is_object($this->formatters[$format])) {
            $this->formatters[$format] = Rnd::createObject($this->formatters[$format]);
        }
    ...
    public function getParser($format)
    {
        static $defaultParsers = [
            self::FORMAT_JSON => 'rnd\httpclient\JsonParser',
            self::FORMAT_URLENCODED => 'rnd\httpclient\UrlEncodedParser',
            self::FORMAT_RAW_URLENCODED => 'rnd\httpclient\UrlEncodedParser',
            self::FORMAT_XML => 'rnd\httpclient\XmlParser',
        ];
        ...
        if (!is_object($this->parsers[$format])) {
            $this->parsers[$format] = Rnd::createObject($this->parsers[$format]);
        }
    ...
    public function createRequest()
    {
        ...
        $config['client'] = $this;
        return Rnd::createObject($config);
    }
    ...
    public function createResponse($content = null, array $headers = [])
    {
        ...
        $config['client'] = $this;
        $response = Rnd::createObject($config);
