...
class BaseFileHelper
{
	 ...
	 * If not set, the value of [[\rnd\base\Application::language]] will be used.
	 * @param string $sourceLanguage the language that the original file is in.
	 * If not set, the value of [[\rnd\base\Application::sourceLanguage]] will be used.
	...
	{
		if ($language === null) {
			$language = Rnd::$app->language;
		}
		if ($sourceLanguage === null) {
			$sourceLanguage = Rnd::$app->sourceLanguage;
		}
	...
	{
		if ($magicFile !== null) {
			$magicFile = Rnd::getAlias($magicFile);
		}
	...
	protected static function loadMimeAliases($aliasesFile)
	{
		...
		$aliasesFile = Rnd::getAlias($aliasesFile);
	 ...
	 * @throws \rnd\base\InvalidParamException if unable to open directory
	 */
	 ...
	 * @throws \rnd\base\Exception if the directory could not be created (i.e. php error due to parallel changes)
	 */
	public static function createDirectory($path, $mode = 0775, $recursive = true)
	{
		...
		try {
			...
			if (!is_dir($path)) {// https://github.com/yiisoft/yii2/issues/9288
				throw new \rnd\base\Exception("Failed to create directory \"$path\": " . $e->getMessage(), $e->getCode(), $e);
			}
		}
		try {
			return chmod($path, $mode);
		} catch (\Exception $e) {
			throw new \rnd\base\Exception("Failed to change permissions for directory \"$path\": " . $e->getMessage(), $e->getCode(), $e);
		}
	}
