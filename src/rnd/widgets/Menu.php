 ...
 *         ]],
 *         ['label' => 'Login', 'url' => ['site/login'], 'visible' => Rnd::$app->user->isGuest],
 *     ],
 * ]);
 * ```
 *
...
class Menu extends Widget
{
	...
	public function run()
	{
		if ($this->route === null && Rnd::$app->controller !== null) {
			$this->route = Rnd::$app->controller->getRoute();
		}
		if ($this->params === null) {
			$this->params = Rnd::$app->getRequest()->getQueryParams();
		}
	...
	protected function isItemActive($item)
	{
		if (isset($item['url'])) {
			$route = $item['url'];
			if ($route == get_home_url())
			{
				return true;
			} else if ($route == get_permalink()) {
				return true;
			}
		}
