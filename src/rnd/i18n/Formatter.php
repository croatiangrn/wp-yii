...
class Formatter extends Object
{
	 ...
	 *
	 * @param mixed $value the value to be formatted
	 * @param boolean $feminine Whether the formatter should be feminine or not
	 * @return string the formatted result.
	 * @throws InvalidParamException if the input value is not numeric or the formatting failed.
	 * @throws InvalidConfigException when the [PHP intl extension](http://php.net/manual/en/book.intl.php) is not available.
	 */
	public function asSpellout($value, $feminine = false)
	{
		if ($value === null) {
			return $this->nullDisplay;
		}
		...
		if ($this->_intlLoaded) {
			if ($feminine) {
				$f = new NumberFormatter($this->locale, NumberFormatter::SPELLOUT);
				$f->setTextAttribute(NumberFormatter::DEFAULT_RULESET, "%spellout-cardinal-feminine");
