...
class I18N extends Component
{
	...
	public function format($message, $params, $language)
	{
		...
		if (preg_match('~{\s*[\w.]+\s*,~u', $message)) {
			...
			if ($result === false) {
				...
				print_r("Formatting message for language '$language' failed with error: $errorMessage. The message being formatted was: $message.", __METHOD__);
	...
	public function getMessageFormatter()
	{
		if ($this->_messageFormatter === null) {
			...
			$this->_messageFormatter = Rnd::createObject($this->_messageFormatter);
		}
	...
	public function getMessageSource($category)
	{
		if (isset($this->translations[$category])) {
			...
			return $this->translations[$category] = Rnd::createObject($source);
		}
		...
		foreach ($this->translations as $pattern => $source) {
			if (strpos($pattern, '*') > 0 && strpos($category, rtrim($pattern, '*')) === 0) {
				...
				return $this->translations[$category] = $this->translations[$pattern] = Rnd::createObject($source);
			}
		}
		...
		if (isset($this->translations['*'])) {
			...
			return $this->translations[$category] = $this->translations['*'] = Rnd::createObject($source);
		}
