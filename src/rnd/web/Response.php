 ...
 * You can access that instance via `Rnd::$app->response`.
 *
 ...
 * @since 2.0
 */
class Response extends \rnd\base\Response
{
	/**
	 ...
	 * {
	 *     $storagePath = Rnd::getAlias('@app/files');
	 *
	 *     // check filename for allowed chars (do not allow ../ to avoid security issue: downloading arbitrary files)
	 *     if (!preg_match('/^[a-z0-9]+\.[a-z0-9]+$/i', $filename) || !is_file("$storagePath/$filename")) {
	 *         throw new \yii\web\NotFoundHttpException('The file does not exists.');
	 *     }
	 *     return Rnd::$app->response->sendFile("$storagePath/$filename", $filename);
	 * }
	 * ```
	 *
	...
	{
		$rangeHeader = Rnd::$app->getRequest()->getHeaders()->get('Range', '-');
		if ($rangeHeader === '-') {
			return [0, $fileSize - 1];
		}
	 ...
	 *
	 * ```php
	 * Rnd::$app->response->xSendFile('/home/user/Pictures/picture1.jpg');
	 * ```
	 *
	 ...
	 *
	 * ```php
	 * return Rnd::$app->getResponse()->redirect($url);
	 * ```
	 *
	 ...
	 *
	 * ```php
	 * Rnd::$app->getResponse()->redirect($url)->send();
	 * return;
	 * ```
	 *
	...
	public function redirect($url, $statusCode = 302, $checkAjax = true)
	{
		...
		if (strpos($url, '/') === 0 && strpos($url, '//') !== 0) {
			$url = Rnd::$app->getRequest()->getHostInfo() . $url;
		}

		if ($checkAjax) {
			if (Rnd::$app->getRequest()->getIsAjax()) {
				if (Rnd::$app->getRequest()->getHeaders()->get('X-Ie-Redirect-Compatibility') !== null && $statusCode === 302) {
					// Ajax 302 redirect in IE does not work. Change status code to 200. See https://github.com/yiisoft/yii2/issues/9670
					$statusCode = 200;
				}
				if (Rnd::$app->getRequest()->getIsPjax()) {
	 ...
	 *
	 * ```php
	 * return Rnd::$app->getResponse()->refresh();
	 * ```
	 *
	...
	{
		return $this->redirect(Rnd::$app->getRequest()->getUrl() . $anchor);
	}
	...
	protected function defaultFormatters()
	{
		return [
			self::FORMAT_HTML => [
				'class' => 'rnd\web\HtmlResponseFormatter',
			],
			self::FORMAT_XML => [
				'class' => 'rnd\web\XmlResponseFormatter',
			],
			self::FORMAT_JSON => [
				'class' => 'rnd\web\JsonResponseFormatter',
			],
			self::FORMAT_JSONP => [
				'class' => 'rnd\web\JsonResponseFormatter',
				'useJsonp' => true,
	...
	protected function prepare()
	{
		...
		if (isset($this->formatters[$this->format])) {
			...
			if (!is_object($formatter)) {
				$this->formatters[$this->format] = $formatter = Rnd::createObject($formatter);
			}
