...
use rnd\widgets\Block;
use rnd\widgets\ContentDecorator;
use rnd\widgets\FragmentCache;
...
class View extends Component
{
	...
	public function init()
	{
		...
		if (is_array($this->theme)) {
			if (!isset($this->theme['class'])) {
				$this->theme['class'] = 'rnd\base\Theme';
			}
			$this->theme = Rnd::createObject($this->theme);
	...
	protected function findViewFile($view, $context = null)
	{
		if (strncmp($view, '@', 1) === 0) {
			// e.g. "@app/views/main"
			$file = Rnd::getAlias($view);
		} elseif (strncmp($view, '//', 2) === 0) {
			// e.g. "//layouts/main"
			$file = Rnd::$app->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
		} elseif (strncmp($view, '/', 1) === 0) {
			// e.g. "/site/index"
			if (Rnd::$app->controller !== null) {
				$file = Rnd::$app->controller->module->getViewPath() . DIRECTORY_SEPARATOR . ltrim($view, '/');
			} else {
		...
		if ($this->beforeRender($viewFile, $params)) {
			...
			if (isset($this->renderers[$ext])) {
				if (is_array($this->renderers[$ext]) || is_string($this->renderers[$ext])) {
					$this->renderers[$ext] = Rnd::createObject($this->renderers[$ext]);
				}
