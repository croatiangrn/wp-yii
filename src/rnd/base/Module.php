...
class Module extends ServiceLocator
{
	...
	public static function getInstance()
	{
		...
		return isset(Rnd::$app->loadedModules[$class]) ? Rnd::$app->loadedModules[$class] : null;
	}
	...
	{
		if ($instance === null) {
			unset(Rnd::$app->loadedModules[get_called_class()]);
		} else {
			Rnd::$app->loadedModules[get_class($instance)] = $instance;
		}
	}
	...
	{
		return Rnd::getAlias('@' . str_replace('\\', '/', $this->controllerNamespace));
	}
	...
	{
		$this->_viewPath = Rnd::getAlias($path);
	}
	...
	{
		$this->_layoutPath = Rnd::getAlias($path);
	}
	 ...
	 * This method calls [[Rnd::setAlias()]] to register the path aliases.
	...
	public function getModule($id, $load = true)
	{
		...
		if (isset($this->_modules[$id])) {
			if ($this->_modules[$id] instanceof Module) {
				...
				/* @var $module Module */
				$module = Rnd::createObject($this->_modules[$id], [$id, $this]);
	 ...
	 * array that can be used to create the module. In the latter case, [[Rnd::createObject()]]
	...
	public function runAction($route, $params = [])
	{
		...
		if (is_array($parts)) {
			...
			$oldController = Rnd::$app->controller;
			Rnd::$app->controller = $controller;
			$result = $controller->runAction($actionID, $params);
			if ($oldController !== null) {
				Rnd::$app->controller = $oldController;
			}
	...
	public function createController($route)
	{
		...
		if (isset($this->controllerMap[$id])) {
			$controller = Rnd::createObject($this->controllerMap[$id], [$id, $this]);
	...
	public function createControllerByID($id)
	{
		...
		if (is_subclass_of($className, 'yii\base\Controller')) {
			$controller = Rnd::createObject($className, [$id, $this]);
