...
class Theme extends Component
{
	...
	{
		$this->_baseUrl = $url === null ? null : rtrim(Rnd::getAlias($url), '/');
	}
	...
	{
		$this->_basePath = Rnd::getAlias($path);
	}
	...
	public function applyTo($path)
	{
		...
		if (empty($pathMap)) {
			...
			$pathMap = [Rnd::$app->getBasePath() => [$basePath]];
		}
		$path = FileHelper::normalizePath($path);
		foreach ($pathMap as $from => $tos) {
			$from = FileHelper::normalizePath(Rnd::getAlias($from)) . DIRECTORY_SEPARATOR;
			if (strpos($path, $from) === 0) {
				$n = strlen($from);
				foreach ((array) $tos as $to) {
					$to = FileHelper::normalizePath(Rnd::getAlias($to)) . DIRECTORY_SEPARATOR;
