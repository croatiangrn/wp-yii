...
use rnd\web\HttpException;
use Rnd;
use rnd\helpers\VarDumper;
use rnd\base\ErrorException;
 ...
 * You can access that instance via `Rnd::$app->errorHandler`.
 *
...
abstract class ErrorHandler extends Component
{
	...
	protected function handleFallbackExceptionMessage($exception, $previousException)
	{
		...
		if (RND_DEBUG) {
			if (PHP_SAPI === 'cli') {
				...
				echo '<pre>' . htmlspecialchars($msg, ENT_QUOTES, Rnd::$app->charset) . '</pre>';
			}
		} else {
	 ...
	 *
	 * @throws \rnd\base\ErrorException
	 * @since 2.0.6
	 */
	 ...
	 * @param \rnd\web\HttpException $exception the exception to be logged
	...
	public function logException($exception)
	{
		...
		if ($exception instanceof HttpException) {
			$category = 'rnd\\web\\HttpException:' . $exception->statusCode;
		} elseif ($exception instanceof \ErrorException) {
			$category .= ':' . $exception->getSeverity();
		}
		Rnd::error($exception, $category);
	}
	...
	{
		if ($exception instanceof Exception && ($exception instanceof UserException || !RND_DEBUG)) {
